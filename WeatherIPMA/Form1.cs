    ...
    public partial class Form1 : Form
    {
        ...
        private void ButtonConfirmarLocal_Click(object sender, EventArgs e)
        {
            ...
            for(int i = 0; i < forecasts.Count; i++)
            {
                ...
                var text = JsonSerializer.Serialize(forecasts[i], options);
                File.WriteAllText(outPath + "\\" + forecasts[i].IdLocal + "-detalhe.json", text);

                // XML (ERROR: first two properties doesnt work)
                XmlSerializer xml = new XmlSerializer(forecasts[i].GetType());
                StringWriter stringWriter = new StringWriter();
                xml.Serialize(stringWriter, forecasts[i]);
                File.WriteAllText(outPath + "\\" + forecasts[i].IdLocal + "-detalhe.xml", stringWriter.ToString());
            }
